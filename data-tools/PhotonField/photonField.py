...
def CMBSpectralNumberDensity(nu):
    """CMB spectral number density [1/m^3/Hz] at frequency nu [1/s]"""
    return CMBSpectralEnergyDensity(nu) / (nu * h_planck)


def IRBSpectralRadiance(nu, fin='Kneiske2004_IRB/z0.txt'):
    """IRB (Kneiske et al. 2004) spectral radiance [W/sr/m^2/Hz] at frequency nu [1/s] and z=0"""
    # convert frequency [Hz] to wavelength [m]
    x = c_light / nu
    
    # load data points
    xKneiske, yKneiske = genfromtxt(fin, unpack=True)
...
def IRBSpectralNumberDensity(nu):
    """IRB (Kneiske et al. 2004) spectral number density [1/m^3/Hz] at frequency nu [1/s]"""
...
if __name__ == "__main__":
    ...
    snd1 = CMBSpectralNumberDensity(nu1)

    nu2 = logspace(-5, 1.5) * eV / h_planck
    sed2 = IRBSpectralEnergyDensity(nu2)
    snd2 = IRBSpectralNumberDensity(nu2)
...
#    savefig('spectralEnergyDensity.png', bbox_inches='tight')
    ...
    loglog()
    legend(frameon=0)
#    savefig('spectralNumberDensity.png', bbox_inches='tight')
