...
inline void write(std::ostream &out, const mpc::Vector3 &vec3) {
	write(out, vec3.getX());
	write(out, vec3.getY());
	write(out, vec3.getZ());
}

inline void write(std::ostream &out, const mpc::ParticleState &state) {
...
inline void write(std::ostream &out, const mpc::InteractionState &s) {
	write(out, s.distance);
	write(out, s.channel);
}

inline void write(std::ostream &out, const mpc::Candidate &candidate) {
...
inline bool read(std::istream &in, mpc::Vector3 &vec3) {
	read(in, vec3[0]);
	read(in, vec3[1]);
	read(in, vec3[2]);
	return (in);
...
inline bool read(std::istream &in, mpc::ParticleState &state) {
...
inline bool read(std::istream &in, const mpc::InteractionState &s) {
	read(in, s.distance);
	read(in, s.channel);
	return (in);
}

inline bool read(std::istream &in, mpc::Candidate &candidate) {
