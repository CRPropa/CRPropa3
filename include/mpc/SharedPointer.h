...
public:
	...
	*/
	shared_ptr() : rawPtr(0), counter(0) { }
	
	/*
		Construct from a raw pointer 
	*/
	shared_ptr(X* raw, Counter* c = 0) : rawPtr(0), counter(0)
	{
		if (raw)
		{
			rawPtr = raw; 
	...
	template <typename Y>
 	explicit shared_ptr(Y* raw, Counter* c = 0) : rawPtr(0), counter(0)
	{
		if (raw)
		{
	...
	/*
		Copy constructor 
	*/
	shared_ptr(const shared_ptr< X >& otherPtr)
	{
	...
	template <typename Y>
	explicit shared_ptr(const shared_ptr< Y >& otherPtr) : rawPtr(0), counter(0)
	{
		acquire(otherPtr.counter);
...
shared_ptr& operator=(const shared_ptr< X >& otherPtr)
{
	if (this != &otherPtr)
	{
		release();
...
template <typename Y>
shared_ptr& operator=(const shared_ptr< Y >& otherPtr)
{
	if ( this != (shared_ptr< X >*) &otherPtr )
	{
		release();
		acquire(otherPtr.counter);
		rawPtr = static_cast<X*> (otherPtr.GetRawPointer());
...
*/
	...
	*/
	...
	operator shared_ptr<Y>()
	{
		...
		//will go out of sync
		return shared_ptr<Y>(rawPtr, counter);
	}
...
bool operator==(const shared_ptr< X >& lptr, const shared_ptr< Y >& rptr) 
{
	return lptr.GetRawPointer() == rptr.GetRawPointer(); 
}

template <typename X, typename Y>
bool operator==(const shared_ptr< X >& lptr, Y* raw) 
{
	return lptr.GetRawPointer() == raw ; 
}

template <typename X>
bool operator==(const shared_ptr< X >& lptr, long num)
{
...
template <typename X>
bool operator&&(const shared_ptr< X >& lptr, bool rval)
{
	return lptr.IsValid() && rval;
}

template <typename X>
bool operator&&(bool lval, const shared_ptr< X >& rptr)
{
...
template <typename X>
bool operator!(const shared_ptr< X >& p)
{
	return (!p.IsValid());
}
...
template <typename X, typename Y>
bool operator< (X* raw, const shared_ptr< Y >& rptr)
{
