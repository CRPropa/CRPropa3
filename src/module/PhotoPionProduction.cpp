...
namespace crpropa {
  ...
  void PhotoPionProduction::init(std::string filename) {
    ...
    while (infile.good()) {
      if (infile.peek() != '#') {
	double z, a, b, c;
	if (!doRedshiftDependent) infile >> a >> b >> c;
	else infile >> z >> a >> b >> c;
	if (infile) {
	  if (doRedshiftDependent && z != zOld) tabRedshifts.push_back(z);
	  if (a < aOld) doReadLorentz = false;
	  if (doReadLorentz) tabLorentz.push_back(pow(10, a));
    ...
    std::cout<<"Read IRB table in "<<tabRedshifts.size()<<"\t"<<tabLorentz.size()<<"\t"<<tabProtonRate.size()<<"\t"<<tabNeutronRate.size()<<std::endl;
    infile.close();
  }
