...
TEST (AdiabaticCooling, UniformField) {
	...
	c.setCurrentStep(10*kpc/c.getVelocity());
	c.setNextStep(10*kpc/c.getVelocity());
	double E = c.current.getEnergy();
	AC.process(&c);

	// Energy is expected to be conserved
	EXPECT_DOUBLE_EQ(c.current.getEnergy(), E);
	EXPECT_DOUBLE_EQ(c.getNextStep()*c.getVelocity(), 10*kpc);
...
TEST (AdiabaticCooling, ConstantSphericalField) {
	...
	c.setCurrentStep(c_light/c.getVelocity());
	c.setNextStep(c_light/c.getVelocity());
	double E = c.current.getEnergy();
	AC.process(&c);

	// Check energy loss and step limitation
	EXPECT_DOUBLE_EQ(c.current.getEnergy(), E/3.);
	EXPECT_DOUBLE_EQ(c.getNextStep()*c.getVelocity(), 0.15*c_light);
