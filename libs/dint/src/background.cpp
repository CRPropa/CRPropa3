...
// 			    0.4, 0.3, 0.5, 0.8, 1.1, 1.3, 1.};
  ...
  double x = C_0/(lLocalEnergy*ELECTRON_MASS*2.42e14)*1.e4; // convert energy into microns
...
double OpticalIR(const double energy) {
  ...
  const double yData[9] = {-30., -1.45, -1.65, -1.74, -0.33, 0.1, 0.2, -0.6,
			   -2.6};
  double result;
  double x = C_0/(energy*ELECTRON_MASS*2.42e14)*1.e4; // convert energy into microns
...
double DustIR(const double energy) {
  ...
  double result;
  double x = C_0/(energy*ELECTRON_MASS*2.42e14)*1.e4;
