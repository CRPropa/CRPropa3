...
public:
	...
	ProgressBar(const std::string &title, unsigned long steps = 0, unsigned long updateSteps = 100) : 
		_steps(steps), _currentCount(0), _maxbarLength(10), _updateSteps(updateSteps), _nextStep(1)
	{
	...
	void update()
	{
		...
		if (_currentCount == _nextStep || _currentCount == _steps)
		{
			...
			if (_currentCount < _steps)
			{
				...
				printf(stringTmpl.c_str(), arrow.c_str(), percentage , "Finish in",int(tToGo/60), int(tToGo)%60, "");
				fflush(stdout);
			}
			else
			{
				...
				printf(stringTmpl.c_str(), fs, percentage, "Needed",int(tElapsed/60), int(tElapsed)%60, s.c_str());
			}
		}
	}
	...
	void setError()
	{
		...
		printf(stringTmpl.c_str(), fs, _currentCount, "Needed",int(tElapsed/60), int(tElapsed)%60, s.c_str());
	}
