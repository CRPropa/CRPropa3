...
R = c.current.getMomentum().getR() / c.current.getCharge() / (10 * nG)


def propagate(tolerance):
    prop = PropagationCK(field, tolerance, 0.1 * kpc)
...
savefig('PropagationCK_xtrajectory', bbox_inches='tight')
...
grid()
text(0.05, 0.95, 'Tolerance 1e-4', ha='left', transform=gca().transAxes)
savefig('PropagationCK_ptrajectory', bbox_inches='tight')
...
grid()
semilogy()
savefig('PropagationCK_pdeviation.png',bbox_inches='tight')
...
semilogy()
ylim(1e-8,1e-1)
savefig('PropagationCK_xdeviation.png',bbox_inches='tight')
