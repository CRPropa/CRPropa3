...
void PhotoPionProduction::setPhotonField(PhotonField field) {
    ...
    if (haveRedshiftDependence) {
        std::cout << "PhotoPionProduction: tabulated redshift dependence not needed for CMB or local fields, switching off" << std::endl;
...
void PhotoPionProduction::performInteraction(Candidate *candidate, bool onProton) const {
    ...
    double momentaList[5][2000]; // momentum list; components: 4-momentum + particle mass
    ...
    double E_threshold;
    double t_blackbody; // t_blackbody of CMB = 2.73 K
    switch (photonField) {
        case CMB:
            E_threshold = 3.72e18 * eV;
            background_flag = 1;
            t_blackbody = 2.73;
            break;
        case Blackbody:
            t_blackbody = getBlackbodyTemperature("PhotoPionProduction/rate_Blackbody.txt");
            E_threshold = 1./100. * 1.6e17 / (3./2. * 8.62e-5 * t_blackbody) * eV; // E_th for the Delta Resonance
            // E_threshold = 3.72e10 * eV;
            background_flag = 3;
            break;
        default:
            E_threshold = 5.83e15 * eV;
            background_flag = 2;
            t_blackbody = 2.73;
            break;
    }
...
#pragma omp critical
    {
        sophiaevent_(nature, Ein, momentaList, particleList, nParticles, z, background_flag, maxRedshift, t_blackbody);
    }
...
double PhotoPionProduction::getBlackbodyTemperature(std::string filename) const {
    double E_BB;
    std::ifstream infile(filename.c_str());
    if (infile.good())
    {
        ...
        std::string str;
        getline(infile, str);
        // extract eV-information from first line
        str.erase(str.begin(), str.begin()+49);
        str.erase(str.end()-2, str.end());
        E_BB = atof(str.c_str());
        infile.close();
    }
    else {
        throw std::runtime_error("PhotoPionProduction: could not open file " + filename + " to read blackbody temperature.");
    }
    return E_BB / 8.62e-5 * 2./3.; // E = 3/2*k_b*T
}
