...
#include "fftw3.h"
void initTurbulence(ref_ptr<VectorGrid> grid, double Brms, double lMin, double lMax, double alpha, int seed, bool helicity, double H, bool bendover, double bendover_lambda) {
		...
		for (size_t iy = 0; iy < n; iy++) {
			for (size_t iz = 0; iz < n2; iz++) {
				...
				if (helicity == true) {
					...
					if (bendover) {
						b *= random.randNorm() * k * bendover_lambda *
							pow(1+k*k*bendover_lambda*bendover_lambda, alpha / 4 - 1./2.);
					} else {
